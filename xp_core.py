"""
XP Calculation Core Module

Provides XP calculation logic and rank management for the Art Learning System.
"""
...
class Constants:
    ...
    TITLES = {
        (1, 5): "Sketcher（スケッチャー）",
        (6, 10): "Line Artist（ラインアーティスト）",
        (11, 15): "Colorist（カラリスト）",
        (16, 20): "Illustrator（イラストレーター）",
        (21, 25): "Creative Designer（クリエイティブデザイナー）",
        (26, 29): "Master Illustrator（マスターイラストレーター）",
        (30, 30): "The Grand Creator（ザ・グランド・クリエータ）",
        (31, 35): "Diamond Art Virtuoso（アート・ヴィルトゥオーソ）",
        (36, 40): "Visual Alchemist（ビジュアル・アルケミスト）",
        (41, 45): "Legendary Creator（伝説のクリエイター）",
...
class XPCalculator:
    ...
    def calculate_acquisition_xp(technique_type: str, evaluation: str) -> int:
        """科目習得のXPを計算します。評価に基づいてスコアが乗算されます。"""
        # 技法固定リストに無い場合は自由投稿作品として扱う
        base_xp = Constants.ACQUISITION_BASE_XP.get(technique_type, Constants.POST_BASE_XP)

        if base_xp is None or base_xp <= 0:
            return 0

        # 評価スコア（A=5, B=4, ..., E=1）を取得
